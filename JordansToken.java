...
public static int getTokenNum(String sToken)
 {
  ...
  if (isReservedPlusOp(sToken)) {
   ...
   type = Print;
  } else if (isReservedEOF(sToken)){
    type = EOF;
  }
  return type;
 }

public static boolean isReservedEOF(String candidate)
{
  int temp = new Integer(candidate);
  return temp == EOF;
}
   ...
   //I'm using longs because wallingford has the specs of int to be twice
   //as big as java allows...unless im missing something? I also put in a 
   //check in case we send a string to isInt that isn't acually an int.
   // - J.
   public static boolean isInt(String num)
   {
     ...
     return ((lowerBound<=temp) && (upperBound>=temp)); 
   }
}
